...
class GA:
    def __init__(self, J_num, State, Machine, PT):
        self.State = State  # 工序数
        self.Machine = Machine
        self.PT = PT  # 各工件对应每道工序的使用时间
        self.J_num = J_num  # 工件数
        self.Pm = 0.1  # 变异概率
        self.Pc = 0.6  # 交叉概率
        self.Pop_size = 20  # 种群数
...
if __name__ == "__main__":
    ...
    for i in range(n):
        ...
        if max(bf) > worst:
            worst = max(bf)
        if min(bf) < res or res == 0:
            res = min(bf)
        if best in bf:
            best_iter_rounds.append(bf.index(best))
            best_apper_times += 1
        _avg.append(np.average(bf))
    print('总运行次数：', n)
    print('最好解：', res)
    print('最坏解：', worst)
    print('平均解：{0:.2f}'.format(np.average(_avg)))
    print('最好解的频率：{0:.2f}'.format(best_apper_times / n))
    print('最好解的平均迭代次数：{0:.2f}'.format(np.average(best_iter_rounds)))
